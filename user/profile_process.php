...
if ($action === 'update_details') {
    ...
    log_activity("Updated personal details.");
    
    // FIX: Redirect to the clean URL /profile
...
if ($action === 'change_password') {
    ...
    if (!password_verify($current_password, $user['password'])) {
        header("Location: /profile?error=current_password_incorrect");
        exit();
    }
    if ($new_password !== $confirm_password) {
        header("Location: /profile?error=passwords_do_not_match");
        exit();
    }
    ...
    // FIX: Redirect to the clean URL /profile
    header("Location: /profile?status=password_updated");
    exit();
}
...
if ($action === 'update_theme') {
    ...
    // FIX: Added the missing log entry
    log_activity("Updated theme preference to '{$theme}'.");

    // FIX: Redirect to the clean URL /profile
    header("Location: /profile?status=theme_updated");
    exit();
}
